...
function InvoiceDocument({ invoice, order, lines }: { invoice: any; order: any; lines: any[] }) {
  ...
  return <Document title={invoice.invoice_number} author="Zen Coffee Lab">
    <Page size="A4" style={styles.page}>
      ...
      <View style={{ marginBottom: 28 }}><Text>{order.shipping_address.firstName} {order.shipping_address.lastName}</Text>{order.shipping_address.company ? <Text>{order.shipping_address.company}</Text> : null}<Text>{order.shipping_address.line1}</Text><Text>{order.shipping_address.postalCode} {order.shipping_address.city} · {order.shipping_address.countryCode}</Text><Text>{order.email}</Text></View>
