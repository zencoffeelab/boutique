...
export function orderConfirmationEmail(order: OrderSnapshot): EmailContent {
  ...
  const shippingLabel = order.shipping_carrier === "Colissimo"
    ? `Colissimo — ${pickup ? (english ? "Pickup Point" : "Point Retrait") : (english ? "Home Delivery" : "Domicile")}`
    : [order.shipping_carrier, order.shipping_service].filter(Boolean).join(" · ");
  const delivery = pickup
    ? `${english ? "Pickup point" : "Point Retrait"}: ${pickup.name}, ${pickup.address1}, ${pickup.postalCode} ${pickup.city}`
    : `${order.shipping_address?.line1 ?? ""}, ${order.shipping_address?.postalCode ?? ""} ${order.shipping_address?.city ?? ""}`;
  const body = `${paragraph(english ? `We have received your payment. Your order ${order.order_number} is confirmed.` : `Nous avons bien reçu votre paiement. Votre commande ${order.order_number} est confirmée.`)}<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:24px 0;font-size:14px;line-height:1.5">${lines}<tr><td style="padding:10px 0">${english ? "Shipping" : "Livraison"} · ${escapeEmailHtml(shippingLabel)}</td><td align="right" style="padding:10px 0">${escapeEmailHtml(formatMoney(order.shipping_charged_cents, order.locale))}</td></tr><tr><td style="padding:14px 0;border-top:2px solid #253021;font-weight:700">Total</td><td align="right" style="padding:14px 0;border-top:2px solid #253021;font-weight:700">${escapeEmailHtml(formatMoney(order.total_cents, order.locale))}</td></tr></table>${paragraph(delivery)}`;
  return {
