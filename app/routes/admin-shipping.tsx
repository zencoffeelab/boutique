    ...
    <dialog className="admin-help-dialog" id="shipping-help" ref={dialogRef} aria-labelledby="shipping-help-title" onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }}>
      <div className="admin-help-dialog__panel">
        ...
        <div className="admin-help-dialog__content">
          <section><h3>À quoi servent les emballages ?</h3><p>Le site transforme automatiquement le panier en un ou plusieurs colis avant d’interroger Sendcloud.</p><dl className="admin-help-definitions"><div><dt>Poids net maximal</dt><dd>Quantité maximale de café dans le carton, sans l’emballage.</dd></div><div><dt>Tare</dt><dd>Poids du carton vide et du calage, ajouté au poids du café.</dd></div><div><dt>Dimensions</dt><dd>Longueur, largeur et hauteur extérieures transmises au transporteur.</dd></div><div><dt>Actif</dt><dd>Autorise l’utilisation de cet emballage dans les nouveaux devis.</dd></div></dl><p><strong>Exemple :</strong> quatre paquets de 200 g représentent 800 g de café. Avec un carton de 180 g, Sendcloud reçoit un colis de 980 g.</p></section>
          ...
          <section><h3>Calcul des tarifs Sendcloud</h3><ol><li>Le poids des paquets est additionné.</li><li>La commande est répartie dans les emballages actifs.</li><li>La tare et les dimensions de chaque colis sont ajoutées.</li><li>Sendcloud renvoie les services et tarifs réels.</li><li>Pour plusieurs colis, les tarifs du même service sont additionnés.</li></ol><p>Le devis est valable 15 minutes. La livraison en point relais est temporairement désactivée.</p></section>
