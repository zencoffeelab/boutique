    ...
    <dialog className="admin-help-dialog" id="shipping-help" ref={dialogRef} aria-labelledby="shipping-help-title" onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }}>
      <div className="admin-help-dialog__panel">
        ...
        <div className="admin-help-dialog__content">
          ...
          <section><h3>Calcul des tarifs transporteur</h3><ol><li>Le poids des paquets est additionné.</li><li>La commande est répartie dans les emballages actifs.</li><li>La tare et les dimensions de chaque colis sont ajoutées.</li><li>En mode comparaison, les offres Colissimo France de Shippo et Sendcloud sont toutes deux proposées et identifiées sur la page commande.</li><li>Les modes Shippo seul et Sendcloud seul restent disponibles avec la variable <code>COLISSIMO_FR_MODE</code>.</li><li>Pour plusieurs colis, les tarifs du même service sont additionnés.</li></ol><p>Mode actif : <strong>{colissimoMode === "compare" ? "comparaison Shippo / Sendcloud" : colissimoMode === "shippo_only" ? "Shippo seul" : "Sendcloud seul"}</strong>. Le devis est valable 15 minutes.</p></section>
