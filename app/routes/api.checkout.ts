...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (!viewer && parsed.data.createAccount) {
    ...
    const { data, error } = await supabase.client.auth.signUp({
      ...
      options: {
        data: { first_name: parsed.data.address.firstName, last_name: parsed.data.address.lastName, phone: parsed.data.address.phone, signup_source: "checkout" },
        emailRedirectTo: `${new URL(request.url).origin}/auth/confirm?next=${encodeURIComponent(accountPath)}`,
      },
    });
  ...
  try { return Response.json(await createCheckout({ cartId: parsed.data.cartId, shippingRateId: parsed.data.shippingRateId, audience, profileId }), { headers: responseHeaders }); }
