...
export async function loader(_: LoaderFunctionArgs) {
  ...
  const paths = [...staticPaths, ...products.flatMap((product) => [`/boutique/${product.slug}`, `/en/shop/${product.slug}`]), ...articles.flatMap((article) => [`/blog/${article.slug}`, `/en/blog/${article.slug}`])];
