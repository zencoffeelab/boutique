...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const status = requestedStatus === "confirmed" || requestedStatus === "pending" || requestedStatus === "buyers" ? requestedStatus : "all";
  if (admin.demo) return { demo: true, query, status, summary: { customers: 0, confirmed: 0, buyers: 0, revenueCents: 0 }, customers: [] };
  ...
  const [profileResult, userResult, orderResult] = await Promise.all([
    client.from("profiles").select("id,first_name,last_name,phone,created_at").eq("role", "customer").is("professional_status", null).order("created_at", { ascending: false }).limit(5_000),
...
export default function AdminCustomers() {
  const { demo, query, status, summary, customers } = useLoaderData<typeof loader>();
  return <AdminShell active="customers">
    ...
    <Card className="admin-professional-section">
      ...
      <CardContent style={{ padding: 0 }}><Table><TableHeader><TableRow><TableHead>Client</TableHead><TableHead>Compte</TableHead><TableHead>Commandes</TableHead><TableHead>Dernière activité</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader><TableBody>{customers.map((customer) => <TableRow key={customer.id}>
        ...
        <TableCell><Badge className={customer.emailConfirmed ? "admin-pro-status admin-pro-status--approved" : "admin-customer-status--pending"}>{customer.emailConfirmed ? "Confirmé" : "À confirmer"}</Badge><br /><small>Inscription : {customer.signupSource}<br />{formatDate(customer.createdAt)}</small></TableCell>
        ...
        <TableCell><div className="admin-customer-actions"><Link className="text-link" to={`/admin/commandes?q=${encodeURIComponent(customer.email)}`}><ShoppingBag aria-hidden="true" /> Voir les commandes</Link><a className="text-link" href={`mailto:${customer.email}`}><UserRound aria-hidden="true" /> Contacter</a></div></TableCell>
