...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  let query = client.from("orders").select("id, order_number, status, email, locale, shipping_address, shipping_carrier, shipping_service, subtotal_cents, shipping_charged_cents, total_cents, paid_at, created_at, order_lines(product_name,variant_label,quantity,unit_price_cents,line_total_cents), shipments(carrier,service,tracking_number,tracking_url,status,status_date)").eq("id", params.id);
