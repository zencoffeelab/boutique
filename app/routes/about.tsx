...
export default function About() {
  ...
  return <><header className="page-hero"><p className="eyebrow">{english ? "Our approach" : "Notre démarche"}</p><h1>{content?.title ?? (english ? "Roast less. Reveal more." : "Torréfier moins. Révéler plus.")}</h1><p className="lede">{lede ?? (english ? "Zen Coffee Lab is a small independent roastery in Tours, born from a desire to make exceptional coffee both precise and approachable." : "Zen Coffee Lab est une micro-torréfaction indépendante à Tours, née de l’envie de rendre les cafés d’exception aussi précis qu’accessibles.")}</p></header>{content ? <EditorialStory content={content.blocks} images={[storyImages[1] ?? storyImages[0] ?? fallbackImage]} splitSections={false} /> : null}<EditorialStory content={paragraph2 ?? [defaults.body]} images={[storyImages[0] ?? storyImages[1] ?? fallbackImage]} imageFirst splitSections={false} /><div className="article-body advice-detail__body advice-detail__action"><Link className="text-link advice-detail__back" to={shopPath}>{english ? "Visit the shop" : "Visiter la boutique"}</Link></div></>;
}
