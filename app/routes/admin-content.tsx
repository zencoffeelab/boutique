...
function HomeFields({ language, statement, values, heroImage }: { language: "Français" | "English"; statement: string; values: readonly (readonly [string, string])[]; heroImage: { url?: string; path?: string; alt?: string } }) {
  ...
  return <fieldset className="admin-home-fields">
    ...
    {language === "Français" ? <div className="field"><AdminImageEditorInput name="homeHeroImage" label={heroImage.url ? "Remplacer l’image du hero" : "Importer l’image du hero"} help="Image affichée derrière « Une torréfaction pensée pour l’origine. » · JPEG, PNG ou WebP · recadrage libre" currentPreviewUrl={heroImage.url || "/media/home-hero-origin.jpg"} defaultAspect="16:9" defaultOutputWidth={1672} /><input type="hidden" name="homeHeroImageUrl" value={String(heroImage.url ?? "")} /><input type="hidden" name="homeHeroImagePath" value={String(heroImage.path ?? "")} /><label>Texte alternatif de l’image<input name="homeHeroImageAltFr" defaultValue={String(heroImage.alt ?? "Chaîne de montagnes enneigées")} maxLength={240} /></label></div> : <div className="field"><label>Texte alternatif de l’image<input name="homeHeroImageAltEn" defaultValue={String(heroImage.alt ?? "Snow-capped mountain range")} maxLength={240} /></label></div>}
