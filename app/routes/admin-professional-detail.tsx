...
function formatAddress(address: Address | null) {
  if (!address?.line1) return "Non renseignée";
...
function hasRecipient(message: MailMessage, email: string) {
  return (
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const [profileResult, authResult] = await Promise.all([
    client
      .from("profiles")
      .select(
        "id,role,professional_status,first_name,last_name,phone,created_at,updated_at",
      )
      .eq("id", memberId.data)
      .maybeSingle(),
    client.auth.admin.getUserById(memberId.data),
  ]);
  if (profileResult.error)
    throw new Response(profileResult.error.message, { status: 500 });
  if (authResult.error)
    throw new Response(authResult.error.message, { status: 500 });
  ...
  const applicationColumns =
    "id,invited_user_id,company_name,country_code,first_name,last_name,email,phone,company_registration_number,vat_number,electronic_billing_address,billing_address,delivery_address,business_type,monthly_volume,comment,status,decision_note,decided_at,created_at";
  const orderColumns =
  ...
  const [
    ...
    client
      ...
      .select(
        "id,quote_number,status,total_cents,created_at,paid_at,storage_path",
      )
      .eq("profile_id", profile.id)
      .order("created_at", { ascending: false })
      .limit(100),
    client
      ...
      .select("id,status,next_delivery_date,created_at,storage_path")
      .eq("profile_id", profile.id)
      .order("created_at", { ascending: false })
      .limit(100),
  ]);
  const error =
    memberApplications.error ??
    emailApplications.error ??
    profileOrders.error ??
    emailOrders.error ??
    receivedMessages.error ??
    sentMessages.error ??
    quotesResult.error ??
  ...
  ] as Application[]).toSorted((left, right) =>
  ...
  const contracts = await Promise.all(
    (contractsResult.data ?? []).map(async (contract) => {
      if (!contract.storage_path) return { ...contract, documentUrl: null };
      const { data } = await client.storage
        .from("professional-contracts")
        .createSignedUrl(contract.storage_path, 300);
      return { ...contract, documentUrl: data?.signedUrl ?? null };
    }),
  );
  return {
    profile,
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (admin.demo)
    return {
      ok: false,
      message: "Les modifications sont désactivées en mode démonstration.",
    };
  const memberId = idSchema.safeParse(params.id);
  const parsed = professionalIdentitySchema.safeParse(
    Object.fromEntries(await request.formData()),
  );
  if (!memberId.success || !parsed.success)
    return { ok: false, message: "Informations d’identité invalides." };
  const client = createServiceSupabase();
  if (!client) return { ok: false, message: "Base de données indisponible." };
  const { data: application, error: applicationError } = await client
    .from("professional_applications")
    .select(
      "id,invited_user_id,country_code,company_name,company_registration_number,vat_number,business_type,monthly_volume",
    )
    .eq("id", parsed.data.applicationId)
    .maybeSingle();
  if (applicationError) return { ok: false, message: applicationError.message };
  if (!application || application.invited_user_id !== memberId.data)
    return { ok: false, message: "Compte professionnel introuvable." };
  const before = {
  ...
  const { error } = await client
    ...
    .eq("invited_user_id", memberId.data);
  if (error) return { ok: false, message: error.message };
  await client
    .from("audit_log")
    .insert({
      actor_id: admin.id === "demo-admin" ? null : admin.id,
      action: "professional_member.identity_updated",
...
export default function AdminProfessionalDetail() {
  ...
  const editableApplication = applications.find(
    (application) => application.invited_user_id === profile.id,
  );
  ...
  return (
    <AdminShell active="professionals">
      <header className="admin-heading admin-professional-detail__heading">
        <div className="admin-professional-detail__identity">
          <p className="eyebrow">Fiche compte professionnel</p>
          <h1>
        ...
        >
          <ArrowLeft aria-hidden="true" /> Professionnels
        </Link>
      </header>
      <nav
        className="admin-professional-detail__tabs"
        aria-label="Sections de la fiche professionnelle"
        role="tablist"
      >
        <button
          type="button"
      ...
      {activeTab === "information" ? (
        <section className="admin-professional-detail__grid" role="tabpanel">
          <Card className="admin-professional-detail__information">
            ...
            <CardContent>
              {latestApplication ? (
                <dl className="admin-professional-detail__list">
                  <div>
                    <dt>Statut</dt>
                    <dd>
                      <Badge
                        className={`admin-pro-status admin-pro-status--${profile.professional_status}`}
                      >
                  ...
                  <div>
                    <dt>Pays</dt>
                    <dd>{latestApplication.country_code || "—"}</dd>
                  </div>
                  <div>
                    <dt>
                      {latestApplication.country_code === "FR"
                        ? "SIRET"
                        : "Company registration number"}
                    </dt>
                    <dd>
                      {latestApplication.company_registration_number || "—"}
                    </dd>
                  </div>
                  <div>
                    <dt>TVA intracommunautaire</dt>
                    <dd>{latestApplication.vat_number || "—"}</dd>
                  </div>
                  <div className="admin-professional-detail__list-wide">
                    <dt>Facturation électronique</dt>
                    <dd>
                      {latestApplication.electronic_billing_address || "—"}
                    </dd>
                  </div>
                  <div className="admin-professional-detail__list-wide">
                    <dt>Adresse de facturation</dt>
                    <dd>{formatAddress(latestApplication.billing_address)}</dd>
                  </div>
                  <div className="admin-professional-detail__list-wide">
                    <dt>Adresse de livraison</dt>
                    <dd>{formatAddress(latestApplication.delivery_address)}</dd>
                  </div>
                </dl>
              ) : (
                <p>
                  Aucune demande professionnelle n’est rattachée à ce compte.
                </p>
              )}
            </CardContent>
          </Card>
          {editableApplication ? (
            <Card className="admin-professional-detail__information">
              <CardHeader>
                ...
                <h2>Modifier l’identité professionnelle</h2>
              </CardHeader>
              <CardContent>
                <Form
                  method="post"
                  className="admin-professional-identity-form"
                >
                  <input
                    type="hidden"
                    name="intent"
                    value="update_professional_identity"
                  />
                  <input
                    type="hidden"
                    name="applicationId"
                    value={editableApplication.id}
                  />
                  {result?.message ? (
                    <p
                      className={
                        result.ok ? "form-message" : "form-message form-error"
                      }
                      role="status"
                    >
                      {result.message}
                  ...
                  <div className="form-grid admin-professional-identity-form__grid">
                    <div className="field">
                      <label>
                        Pays
                        <select
                          name="countryCode"
                          defaultValue={
                            editableApplication.country_code ?? "FR"
                          }
                          required
                        >
                          {SHIPPING_COUNTRY_CODES.map((countryCode) => (
                    ...
                    <div className="field">
                      <label>
                        ...
                        <input
                          ...
                          defaultValue={
                            editableApplication.company_registration_number ??
                            ""
                          }
                          minLength={2}
                          maxLength={80}
                    ...
                    <div className="field">
                      <label>
                        ...
                        <input
                          name="companyName"
                          defaultValue={editableApplication.company_name}
                          minLength={2}
                          maxLength={160}
                          required
                        />
                    ...
                    <div className="field">
                      <label>
                        ...
                        <input
                          name="vatNumber"
                          defaultValue={editableApplication.vat_number ?? ""}
                          maxLength={40}
                        />
                      </label>
                    </div>
                    <div className="field">
                      <label>
                        ...
                        <select
                          name="businessType"
                          defaultValue={editableApplication.business_type}
                          required
                        >
                    ...
                    <div className="field">
                      <label>
                        ...
                        <select
                          name="monthlyVolume"
                          defaultValue={editableApplication.monthly_volume}
                          required
                        >
      ...
      {activeTab === "information" ? (
        <Card
          className="admin-professional-section admin-professional-detail__panel"
          role="tabpanel"
        >
          <CardHeader>
            <p className="eyebrow">Besoins et demandes</p>
              ...
              <div className="admin-professional-detail__timeline">
                {applications.map((application) => (
                  <article key={application.id}>
                    ...
                    {application.comment ? <p>{application.comment}</p> : null}
                    {application.decision_note ? (
                      <small>
                        Note de décision : {application.decision_note}
                      </small>
                    ) : null}
                  </article>
                ))}
      ...
      {activeTab === "documents" ? (
        ...
        >
          <CardHeader>
            <p className="eyebrow">Dossier du compte</p>
            <h2>Documents liés au compte</h2>
          </CardHeader>
          <CardContent style={{ padding: 0 }}>
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead aria-label="Télécharger" />
                </TableRow>
              </TableHeader>
              <TableBody>
                {orders
                  ...
                  .map((order) => (
                    <TableRow key={`invoice-${order.id}`}>
                      <TableCell>
                        <strong>Facture {order.order_number}</strong>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {formatMoney(order.total_cents, "fr-FR")}
                      </TableCell>
                      <TableCell>
                        <a
                          className="text-link"
                ...
                {quotes.map((quote) => (
                  <TableRow key={`quote-${quote.id}`}>
                    <TableCell>
                      <strong>Devis {quote.quote_number}</strong>
                    </TableCell>
                    <TableCell>{formatDate(quote.created_at)}</TableCell>
                    <TableCell>{quote.status}</TableCell>
                    <TableCell>
                      {formatMoney(quote.total_cents, "fr-FR")}
                    </TableCell>
                    <TableCell>
                      <a
                        className="text-link"
                  ...
                  <TableRow key={`contract-${contract.id}`}>
                    ...
                    <TableCell>
                      ...
                      {new Intl.DateTimeFormat("fr-FR", {
                        dateStyle: "medium",
                      }).format(new Date(contract.next_delivery_date))}
                    </TableCell>
                    <TableCell>
            ...
            contracts.length ? null : (
              <p className="admin-empty-state">
                Aucun document lié à ce compte.
              </p>
            )}
          </CardContent>
        </Card>
