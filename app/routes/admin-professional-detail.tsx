...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const [memberApplications, emailApplications, profileOrders, emailOrders, receivedMessages, sentMessages, quotesResult, contractsResult] = await Promise.all([
    ...
    client.from("professional_quotes").select("id,quote_number,status,total_cents,created_at,paid_at,storage_path").eq("profile_id", profile.id).order("created_at", { ascending: false }).limit(100),
    client.from("professional_contracts").select("id,status,next_delivery_date,created_at,storage_path").eq("profile_id", profile.id).order("created_at", { ascending: false }).limit(100),
  ]);
  const error = memberApplications.error ?? emailApplications.error ?? profileOrders.error ?? emailOrders.error ?? receivedMessages.error ?? sentMessages.error ?? quotesResult.error ?? contractsResult.error;
  ...
  const contracts = await Promise.all((contractsResult.data ?? []).map(async (contract) => {
    ...
    const { data } = await client.storage.from("professional-contracts").createSignedUrl(contract.storage_path, 300);
    return { ...contract, documentUrl: data?.signedUrl ?? null };
  }));
  return { profile, email, applications, orders, messages, quotes: quotesResult.data ?? [], contracts };
}
...
export default function AdminProfessionalDetail() {
  const { profile, email, applications, orders, messages, quotes, contracts } = useLoaderData<typeof loader>();
  const result = useActionData<typeof action>();
  const latestApplication = applications[0];
  const editableApplication = applications.find((application) => application.invited_user_id === profile.id);
  const [activeTab, setActiveTab] = useState<"information" | "documents" | "orders" | "messages">("information");
  return <AdminShell active="professionals">
    ...
    <nav className="admin-professional-detail__tabs" aria-label="Sections de la fiche professionnelle" role="tablist">
      ...
      <button type="button" role="tab" aria-selected={activeTab === "documents"} className={activeTab === "documents" ? "is-active" : ""} onClick={() => setActiveTab("documents")}>Documents <span>{orders.filter((order) => order.paid_at).length + quotes.length + contracts.length}</span></button>
    ...
    {activeTab === "information" ? <Card className="admin-professional-section admin-professional-detail__panel" role="tabpanel"><CardHeader><p className="eyebrow">Besoins et demandes</p><h2>Historique des demandes</h2></CardHeader><CardContent>{applications.length ? <div className="admin-professional-detail__timeline">{applications.map((application) => <article key={application.id}><div><Badge className={`admin-pro-status admin-pro-status--${application.status}`}>{statusLabel(application.status)}</Badge><time dateTime={application.created_at}>{formatDate(application.created_at)}</time></div><strong>{application.business_type} · {application.monthly_volume}</strong>{application.comment ? <p>{application.comment}</p> : null}{application.decision_note ? <small>Note de décision : {application.decision_note}</small> : null}</article>)}</div> : <p>Aucune demande enregistrée.</p>}</CardContent></Card> : null}
    {activeTab === "documents" ? <Card className="admin-professional-section admin-professional-detail__panel" role="tabpanel"><CardHeader><p className="eyebrow">Dossier du compte</p><h2>Documents liés au compte</h2></CardHeader><CardContent style={{ padding: 0 }}><Table><TableHeader><TableRow><TableHead>Document</TableHead><TableHead>Date</TableHead><TableHead>Statut</TableHead><TableHead>Montant / détail</TableHead><TableHead aria-label="Télécharger" /></TableRow></TableHeader><TableBody>{orders.filter((order) => order.paid_at).map((order) => <TableRow key={`invoice-${order.id}`}><TableCell><strong>Facture {order.order_number}</strong></TableCell><TableCell>{formatDate(order.paid_at ?? order.created_at)}</TableCell><TableCell>Disponible</TableCell><TableCell>{formatMoney(order.total_cents, "fr-FR")}</TableCell><TableCell><a className="text-link" href={`/api/orders/${order.id}/invoice`}>Télécharger</a></TableCell></TableRow>)}{quotes.map((quote) => <TableRow key={`quote-${quote.id}`}><TableCell><strong>Devis {quote.quote_number}</strong></TableCell><TableCell>{formatDate(quote.created_at)}</TableCell><TableCell>{quote.status}</TableCell><TableCell>{formatMoney(quote.total_cents, "fr-FR")}</TableCell><TableCell><a className="text-link" href={`/api/professional-quotes/${quote.id}/pdf`}>Télécharger</a></TableCell></TableRow>)}{contracts.map((contract) => <TableRow key={`contract-${contract.id}`}><TableCell><strong>Contrat</strong></TableCell><TableCell>{formatDate(contract.created_at)}</TableCell><TableCell>{contract.status}</TableCell><TableCell>Prochaine livraison : {new Intl.DateTimeFormat("fr-FR", { dateStyle: "medium" }).format(new Date(contract.next_delivery_date))}</TableCell><TableCell>{contract.documentUrl ? <a className="text-link" href={contract.documentUrl} target="_blank" rel="noreferrer">Télécharger</a> : "À déposer"}</TableCell></TableRow>)}</TableBody></Table>{orders.some((order) => order.paid_at) || quotes.length || contracts.length ? null : <p className="admin-empty-state">Aucun document lié à ce compte.</p>}</CardContent></Card> : null}
