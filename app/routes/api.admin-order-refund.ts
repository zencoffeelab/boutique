...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const refund = await createStripe(config.STRIPE_SECRET_KEY).refunds.create({ payment_intent: payment.provider_payment_intent_id, amount: parsed.data.amountCents, reason: "requested_by_customer", metadata: { order_id: params.id, internal_reason: parsed.data.reason } }, { idempotencyKey: `refund:${params.id}:${payment.refunded_cents}:${parsed.data.amountCents}` });
