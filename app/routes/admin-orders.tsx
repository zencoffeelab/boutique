...
import { getLabelRefundStates, type LabelRefundState } from "~/services/label-refunds.server";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const { data, error } = await query; if (error) throw new Response(error.message, { status: 500 });
  const orders = data ?? [];
  const transactionIds = [...new Set(orders.flatMap((order) => order.shipments ?? []).flatMap((shipment: any) => shipment.shippo_transaction_id ? [shipment.shippo_transaction_id] : []))];
  const refunds = await getLabelRefundStates(transactionIds);
  return { demo: false, orders: orders.map((order) => ({ ...order, shipments: (order.shipments ?? []).map((shipment: any) => ({ ...shipment, label_refund: shipment.shippo_transaction_id ? refunds[shipment.shippo_transaction_id] : undefined })) })), search, status };
}
...
export function orderContentsLabel(lines: readonly { quantity: number; product_name: string; variant_label: string }[]) {
  if (lines.length === 0) return "Aucun article";
  return lines.map((line) => `${line.quantity} × ${line.product_name} · ${line.variant_label}`).join(" | ");
}

function ShipmentActions({ orderId, shipment }: { orderId: string; shipment: any }) {
  const fetcher = useFetcher<{ ok?: boolean; message?: string | null; refund?: LabelRefundState }>();
  const refund = fetcher.data?.refund ?? shipment.label_refund as LabelRefundState | undefined;
  const pending = refund && ["REQUESTING", "QUEUED", "PENDING"].includes(refund.status);
  const canRequest = Boolean(shipment.shippo_transaction_id && shipment.label_url && !refund && ["", "UNKNOWN", "PRE_TRANSIT"].includes(String(shipment.status ?? "").toUpperCase()));
  const canRefresh = Boolean(pending);
  const formAction = `/api/admin/orders/${orderId}/shipments/${shipment.id}/refund-label`;
  return <article className="admin-shipment">
    <div><strong>Colis {Number(shipment.parcel_index) + 1}</strong><small>{shipment.carrier} · {shipment.service} · {formatMoney(shipment.actual_cost_cents, "fr-FR")}</small></div>
    <div className="admin-shipment__actions">
      {shipment.label_url && !refund ? <a className="text-link" href={shipment.label_url} target="_blank" rel="noreferrer">Télécharger l’étiquette <ExternalLink aria-hidden="true" /></a> : null}
      {refund ? <span className={`ui-badge label-refund-status label-refund-status--${refund.status.toLowerCase()}`}>{refundStatusLabels[refund.status]}</span> : null}
      {canRequest || canRefresh ? <fetcher.Form method="post" action={formAction} onSubmit={(event) => {
        if (canRequest && !window.confirm("Demander le remboursement réel de cette étiquette ? Elle ne devra plus jamais être utilisée.")) event.preventDefault();
      }}><button className={canRequest ? "ui-button ui-button--danger ui-button--sm" : "ui-button ui-button--outline ui-button--sm"} type="submit" disabled={fetcher.state !== "idle"}>{fetcher.state !== "idle" ? "Traitement…" : canRequest ? "Rembourser l’étiquette" : "Actualiser le remboursement"}</button></fetcher.Form> : null}
      {refund?.message || fetcher.data?.message ? <small className="form-error">{refund?.message ?? fetcher.data?.message}</small> : null}
      {pending ? <small>Ne déposez plus ce colis avec cette étiquette. Le crédit Shippo peut prendre plusieurs jours.</small> : null}
    </div>
  </article>;
}
