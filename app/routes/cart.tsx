...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const products = canUseProfessionalSampleSet
    ? await getProfessionalCartProductsWithSampleSet()
    : await getProducts({ status: "published", audience });
  return { locale: getLocale(request), products };
}
...
function CartProductImage({ product, locale }: { product: Product; locale: Locale }) {
  const translation = product.translations[locale];
  if (!product.thumbnailLabelUrl)
    return <img src={product.media[0]?.url} alt={translation.name} width="110" height="110" />;
  return <div className="cart-line__product-image" style={{ "--product-thumbnail-color": product.thumbnailBackgroundColor } as CSSProperties}>
    <ProductPackArtwork packClassName="cart-line__product-pack" labelClassName="cart-line__product-label" labelUrl={product.thumbnailLabelUrl} alt={translation.name} loading="lazy" />
  </div>;
}
...
export default function Cart() {
  ...
  return <>
    <header className="page-hero"><p className="eyebrow">{english ? "Summary" : "Récapitulatif"}</p><h1>{english ? "Your cart" : "Votre panier"}</h1></header>
