...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [products, content] = await Promise.all([audience === "professional" ? getProducts({ status: "published", audience: "professional" }) : Promise.resolve([]), getContentPage("professionnel", locale)]);
...
export default function Professional() {
  ...
  return <>
    ...
    {approved ? <section className="section page-shell"><div className="product-grid">{products.map((product) => <ProductCard key={product.id} product={product} locale={locale} />)}</div><p className="admin-notice" style={{ marginTop: "2rem" }}>{english ? "Professional prices are visible only in this authenticated session." : "Les prix professionnels ne sont visibles que dans cette session authentifiée."}</p></section> : <>
