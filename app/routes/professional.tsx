...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  const connected = Boolean(data?.approved || data?.admin);
  const content = connected ? data?.connectedContent : data?.content;
  return pageMeta(
    content?.seoTitle ?? (data?.locale === "en-GB" ? "Coffee for professionals | Zen Coffee Lab" : "Café pour professionnels | Zen Coffee Lab"),
    content?.seoDescription ?? (data?.locale === "en-GB" ? "Specialty coffee and support for cafés, restaurants and resellers." : "Cafés de spécialité et accompagnement pour coffee shops, restaurants et revendeurs."),
    data?.locale === "en-GB" ? "/en/professional" : "/professionnel",
  );
};
