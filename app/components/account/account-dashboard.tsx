...
import { useEffect, useState, type FormEventHandler, type PropsWithChildren, type ReactNode } from "react";
...
export function AccountNavigation({ english, orderCount, addressCount, professional = false, contractual = false, documentCount = 0, communicationCount = 0, activeSection = "orders", onSelect = () => undefined }: { english: boolean; orderCount: number; addressCount: number; professional?: boolean; contractual?: boolean; documentCount?: number; communicationCount?: number; activeSection?: AccountSectionId; onSelect?: (section: AccountSectionId) => void }) {
  const tab = (section: AccountSectionId, content: ReactNode) => <button id={`account-tab-${section}`} className={activeSection === section ? "is-active" : undefined} type="button" role="tab" aria-selected={activeSection === section} aria-controls={`account-${section}`} onClick={() => onSelect(section)}>{content}</button>;
  return <nav className="account-anchor-nav account-anchor-nav--tabs" role="tablist" aria-label={english ? "My account sections" : "Sections de mon compte"}>
    {contractual ? tab("upcoming-order", <><CalendarClock aria-hidden="true" /><span><strong>{english ? "Your next order" : "Votre prochaine commande"}</strong><small>{english ? "Contract delivery" : "Livraison contractuelle"}</small></span><ChevronRight aria-hidden="true" /></>) : null}
    {tab("orders", <><Package aria-hidden="true" /><span><strong>{english ? "Orders" : "Commandes"}</strong><small>{english ? "Invoices and tracking" : "Factures et suivis"}</small></span><em>{orderCount}</em><ChevronRight aria-hidden="true" /></>)}
    {professional ? tab("communication", <><Mail aria-hidden="true" /><span><strong>{english ? "Communication" : "Communication"}</strong><small>{english ? "Your email exchanges" : "Vos échanges par e-mail"}</small></span><em>{communicationCount}</em><ChevronRight aria-hidden="true" /></>) : null}
    {professional ? tab("documents", <><FileText aria-hidden="true" /><span><strong>{english ? "Documents" : "Documents"}</strong><small>{english ? "Quotes and invoices" : "Devis et factures"}</small></span><em>{documentCount}</em><ChevronRight aria-hidden="true" /></>) : null}
    {tab("addresses", <><MapPin aria-hidden="true" /><span><strong>{english ? "Addresses" : "Adresses"}</strong><small>{english ? "Saved delivery details" : "Coordonnées enregistrées"}</small></span><em>{addressCount}</em><ChevronRight aria-hidden="true" /></>)}
    {tab("settings", <><ShieldCheck aria-hidden="true" /><span><strong>{english ? "Settings" : "Paramètres"}</strong><small>{english ? "Access and security" : "Accès et sécurité"}</small></span><ChevronRight aria-hidden="true" /></>)}
  </nav>;
}
...
function sectionVisibility(activeSection: AccountSectionId, section: AccountSectionId) {
...
function AccountSections({ data, result, mode, activeSection, onNavigate }: { data: AccountDashboardData; result?: AccountActionFeedback | null; mode: "page" | "drawer"; activeSection: AccountSectionId; onNavigate?: () => void }) {
  ...
  return <main className="account-sections">
    ...
    <section className="account-section" id={`${prefix}-orders`} aria-labelledby={drawer ? "account-drawer-tab-orders" : "account-tab-orders"} role="tabpanel" hidden={sectionVisibility(activeSection, "orders")}>
    ...
    {viewer.profile?.professional_account_type === "contractual" ? <section className="account-section" id={`${prefix}-upcoming-order`} aria-labelledby={drawer ? "account-drawer-tab-upcoming-order" : "account-tab-upcoming-order"} role="tabpanel" hidden={sectionVisibility(activeSection, "upcoming-order")}>
    ...
    {professional ? <section className="account-section" id={`${prefix}-documents`} aria-labelledby={drawer ? "account-drawer-tab-documents" : "account-tab-documents"} role="tabpanel" hidden={sectionVisibility(activeSection, "documents")}>
    ...
    {professional ? <section className="account-section" id={`${prefix}-communication`} aria-labelledby={drawer ? "account-drawer-tab-communication" : "account-tab-communication"} role="tabpanel" hidden={sectionVisibility(activeSection, "communication")}>
    ...
    <section className="account-section" id={`${prefix}-addresses`} aria-labelledby={drawer ? "account-drawer-tab-addresses" : "account-tab-addresses"} role="tabpanel" hidden={sectionVisibility(activeSection, "addresses")}>
    ...
    <section className="account-section" id={`${prefix}-settings`} aria-labelledby={drawer ? "account-drawer-tab-settings" : "account-tab-settings"} role="tabpanel" hidden={sectionVisibility(activeSection, "settings")}>
...
export function AccountDashboard({ data, result, mode = "page", activeSection = "orders", onNavigate }: { data: AccountDashboardData; result?: AccountActionFeedback | null; mode?: "page" | "drawer"; activeSection?: AccountSectionId; onNavigate?: () => void }) {
  ...
  const [activePageSection, setActivePageSection] = useState<AccountSectionId>(activeSection === "orders" && viewer.profile?.professional_account_type === "contractual" ? "upcoming-order" : activeSection);
  const selectPageSection = (section: AccountSectionId) => {
    ...
    window.requestAnimationFrame(() => {
      ...
      (title?.closest(".account-section__heading") ?? title)?.scrollIntoView({ behavior: "smooth", block: "start" });
    });
  };
  ...
  return <>
    ...
    <div className="page-shell account-page-shell">
      <aside className="account-sidebar">
        ...
        <AccountNavigation english={english} orderCount={orders.length} addressCount={addresses.length} professional={viewer.profile?.professional_status === "approved"} contractual={viewer.profile?.professional_account_type === "contractual"} documentCount={orders.filter((order) => order.paid_at).length + professionalQuotes.length} communicationCount={professionalMessages.length} activeSection={activePageSection} onSelect={selectPageSection} />
      </aside>
      <AccountSections data={data} result={result} mode={mode} activeSection={activePageSection} />
    </div>
  </>;
}
