...
export function AccountDrawer({
  ...
  const lastActionDataRef = useRef<AccountActionFeedback | undefined>(
    undefined,
  );
  const [closing, setClosing] = useState(false);
  const [activeSection, setActiveSection] =
    useState<AccountSectionId>("communication");
  const accountFetcher = useFetcher<AccountDrawerResponse>({
    key: "account-drawer-data",
  });
  const actionFetcher = useFetcher<AccountActionFeedback>({
  ...
  const mfaChallengeRequired = Boolean(
    data?.mfa?.verifiedFactors.length && data.mfa.currentLevel !== "aal2",
  );
  const visibleTabs =
    data?.viewer?.profile?.professional_status === "approved"
      ? tabs.filter(
          (tab) =>
            data.viewer?.profile?.professional_account_type === "contractual" ||
  ...
  useEffect(() => {
    if (
      ...
      !actionFetcher.data ||
      actionFetcher.data === lastActionDataRef.current
    )
      return;
  ...
  const displayName = data?.viewer
    ? [data.viewer.profile?.first_name, data.viewer.profile?.last_name]
        .filter(Boolean)
        .join(" ")
    : "";
    ...
    >
      <div className="account-drawer__panel">
        <header className="account-drawer__header">
          <div>
            <p className="eyebrow">
              {english ? "Private space" : "Espace privé"}
            </p>
            <h2 id="account-drawer-title">
              {displayName || (english ? "My account" : "Mon compte")}
            </h2>
            {data?.viewer?.user.email ? (
              <small>{data.viewer.user.email}</small>
            ) : null}
          </div>
          <button
            ...
            type="button"
            onClick={onClose}
            aria-label={english ? "Close my account" : "Fermer mon compte"}
            autoFocus
          >
            <X aria-hidden="true" />
          </button>
        ...
        {data?.viewer && !mfaChallengeRequired ? (
          <div
            className={`account-drawer__tabs account-drawer__tabs--${visibleTabs.length}`}
            role="tablist"
            aria-label={
              english ? "My account sections" : "Rubriques de mon compte"
            }
          >
            {visibleTabs.map((tab) => (
              <button
                ...
                onKeyDown={(event) => {
                  ...
                  window.requestAnimationFrame(() =>
                    document
                      .getElementById(`account-drawer-tab-${nextTab.id}`)
                      ?.focus(),
                  );
                }}
                key={tab.id}
        ...
        <div className="account-drawer__content">
          {accountFetcher.state === "loading" && !data ? (
            <div className="account-drawer__loading" role="status">
          ...
          {data?.viewer && mfaChallengeRequired ? (
            <div className="account-drawer__empty">
              <p>
                {english
                  ? "Complete two-factor authentication to open this protected account."
                  : "Terminez la double authentification pour ouvrir ce compte protégé."}
              </p>
              <Link
                className="button button--dark"
                to={accountPath}
                onClick={onClose}
              >
                {english ? "Verify my account" : "Vérifier mon compte"}
              </Link>
            </div>
          ) : null}
          {data?.viewer && !mfaChallengeRequired ? (
            <AccountDashboard
              data={data as AccountDashboardData}
              result={actionFetcher.data}
              mode="drawer"
        ...
        <footer className="account-drawer__footer">
          ...
          >
            ...
            {english ? "Full account page" : "Voir la page complète"}
          </Link>
