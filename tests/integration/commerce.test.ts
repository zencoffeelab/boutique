...
describe("commerce boundaries", () => {
  ...
  it("quotes real parcel weight in mock mode without exposing Shippo identifiers", async () => {
    ...
    expect(response.status).toBe(200); expect(data.parcels[0].shippingWeightGrams).toBe(580); expect(data.rates[0]).toMatchObject({ provider: "mock", carrier: "Colissimo", deliveryMethod: "home" }); expect(data.rates[0]).not.toHaveProperty("shippoRateIds");
  });
  it("reports a stock conflict instead of a carrier outage", async () => {
    const product = (await getProducts({ status: "published" }))[0]; const variant = product.variants[0];
    const request = new Request("http://localhost/api/shipping/quote", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ cartId: crypto.randomUUID(), locale: "fr-FR", lines: [{ productId: product.id, variantId: variant.id, audience: "retail", quantity: 100 }], address: { firstName: "Ada", lastName: "Lovelace", company: "", email: "ada@example.com", phone: "0600000000", line1: "1 rue du Café", line2: "", postalCode: "37000", city: "Tours", countryCode: "FR" } }) });
    const response = await shippingQuoteAction({ request, params: {}, context: {} } as never); const data = await response.json();
    expect(response.status).toBe(409); expect(data.message).toContain("stock"); expect(data.message).not.toContain("transporteur");
  });
  it("returns a pickup rate for a selected Colissimo Point Retrait", async () => {
    const product = (await getProducts({ status: "published" }))[0]; const variant = product.variants[0];
    ...
    const response = await shippingQuoteAction({ request, params: {}, context: {} } as never); const data = await response.json();
    expect(response.status).toBe(200); expect(data.rates[0]).toMatchObject({ deliveryMethod: "pickup", carrier: "Colissimo", pickupPoint: { id: "370000" } });
  });
});
