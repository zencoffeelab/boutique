...
describe("professional application", () => {
  const valid = { companyName: "Coffee Club", countryCode: "FR", companyRegistrationNumber: "12345678900012", lastName: "Doe", firstName: "Jane", email: "jane@example.com", phone: "0600000000", billingLine1: "1 rue du Café", billingPostalCode: "37000", billingCity: "Tours", businessType: "Coffee shop", monthlyVolume: "11-50 kg", locale: "fr-FR", privacyConsent: true };
  it("accepts every planned field and volume", () => expect(professionalApplicationSchema.safeParse(valid).success).toBe(true));
  it("accepts empty optional phone and VAT numbers", () => expect(professionalApplicationSchema.safeParse({ ...valid, phone: "", vatNumber: "" }).success).toBe(true));
  it("requires every part of a delivery address when one is supplied", () => expect(professionalApplicationSchema.safeParse({ ...valid, deliveryLine1: "2 rue des Fleurs" }).success).toBe(false));
  it("accepts a complete delivery recipient and address", () => expect(professionalApplicationSchema.safeParse({ ...valid, deliveryLastName: "Smith", deliveryFirstName: "Alex", deliveryLine1: "2 rue des Fleurs", deliveryPostalCode: "37000", deliveryCity: "Tours" }).success).toBe(true));
