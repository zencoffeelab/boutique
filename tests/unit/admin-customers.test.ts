...
describe("retail customer administration", () => {
  it("joins customer accounts with paid orders and keeps the signup source", () => {
    const customers = buildRetailCustomers(
      [{ id: "customer-1", first_name: "Ada", last_name: "Lovelace", phone: "0600000000", created_at: "2026-07-20T10:00:00.000Z" }],
    ...
    expect(customers[0]).toMatchObject({ email: "ADA@example.com", signupSource: "Commande", emailConfirmed: true, orderCount: 1, totalSpentCents: 2_500, location: "Tours · FR" });
  });
});
